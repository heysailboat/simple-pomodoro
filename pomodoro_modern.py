...
class PomodoroApp(tk.Tk):
    ...
    def _build_style(self) -> None:
        ...
        style.configure("Block.TLabel", font=("Helvetica", 11, "bold"))
        style.configure("Timer.TLabel", font=("Helvetica", 34, "bold"))
        style.configure("Muted.TLabel", foreground="#777777", font=("Helvetica", 9))
        style.configure("Primary.TButton", font=("Helvetica", 9, "bold"))
        style.configure("Compact.TButton", font=("Helvetica", 9))
        style.configure("Editor.TEntry", padding=2)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(header, textvariable=self.status_var, style="Muted.TLabel").pack(side="right", pady=2)
        ...
        ttk.Label(timer, textvariable=self.current_block_var, style="Block.TLabel").pack()

        self.timer_var = tk.StringVar(value="25:00")
        ttk.Label(timer, textvariable=self.timer_var, style="Timer.TLabel").pack(pady=(0, 0))

        self.cycle_var = tk.StringVar()
        ttk.Label(timer, textvariable=self.cycle_var, style="Muted.TLabel").pack(pady=(0, 4))

        buttons = ttk.Frame(timer)
        buttons.pack()
        self.start_button = ttk.Button(buttons, text="Start", width=8, style="Primary.TButton", command=self._toggle_running)
        self.start_button.pack(side="left", padx=2)
        ttk.Button(buttons, text="Skip", width=8, style="Compact.TButton", command=self._skip).pack(side="left", padx=2)
        ttk.Button(buttons, text="Reset", width=8, style="Compact.TButton", command=self._reset).pack(side="left", padx=2)

        # Sequence header + editor
        seq_header = ttk.Frame(self)
        seq_header.pack(fill="x", pady=(4, 3))
        ttk.Label(seq_header, text="Sequence", style="Section.TLabel").pack(side="left")
        ttk.Label(seq_header, text="double-click fields to edit", style="Muted.TLabel").pack(side="right")
        ...
        controls.pack(side="right", fill="y", padx=(7, 0))
        ttk.Button(controls, text="+", width=3, command=self._add_block).pack(pady=1)
        ttk.Button(controls, text="−", width=3, command=self._delete_selected).pack(pady=1)
        ttk.Separator(controls).pack(fill="x", pady=4)
        ttk.Button(controls, text="↑", width=3, command=lambda: self._move_selected(-1)).pack(pady=1)
        ttk.Button(controls, text="↓", width=3, command=lambda: self._move_selected(1)).pack(pady=1)
        ...
        self.cycles_spin = tk.Spinbox(settings, from_=0, to=9999, width=5, bd=1, relief="solid")
        self.cycles_spin.pack(side="left", padx=(5, 4))
