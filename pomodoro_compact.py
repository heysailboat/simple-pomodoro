...
class PomodoroApp(tk.Tk):
    ...
    def _build_style(self) -> None:
        ...
        style.configure("Block.TLabel", font=("Helvetica", 10, "bold"))
        style.configure("Timer.TLabel", font=("Helvetica", 29, "bold"))
        style.configure("Muted.TLabel", foreground="#777777", font=("Helvetica", 8))
        style.configure("Primary.TButton", font=("Helvetica", 8, "bold"), padding=(7, 2))
        style.configure("Compact.TButton", font=("Helvetica", 8), padding=(6, 2))
        style.configure("Section.TLabel", font=("Helvetica", 9, "bold"))

    def _build_ui(self) -> None:
        # Compact window: most of the information fits in one screen without
        # turning the editor into a miniature spreadsheet application.
        self.minsize(420, 285)
        self.geometry("450x315")
        self.configure(padx=8, pady=7)

        header = ttk.Frame(self)
        header.pack(fill="x", pady=(0, 3))
        ttk.Label(header, text="Pomodoro", style="Title.TLabel").pack(side="left")

        self.status_var = tk.StringVar(value="Ready")
        ttk.Label(header, textvariable=self.status_var, style="Muted.TLabel").pack(side="right", pady=1)

        # Timer/control strip. Keeping these on one line saves a surprising
        # amount of vertical space.
        timer = ttk.Frame(self)
        timer.pack(fill="x", pady=(0, 5))

        left = ttk.Frame(timer)
        left.pack(side="left", fill="y", padx=(1, 8))
        self.current_block_var = tk.StringVar()
        ttk.Label(left, textvariable=self.current_block_var, style="Block.TLabel").pack(anchor="w")
        self.cycle_var = tk.StringVar()
        ttk.Label(left, textvariable=self.cycle_var, style="Muted.TLabel").pack(anchor="w", pady=(1, 0))

        center = ttk.Frame(timer)
        center.pack(side="left", expand=True)
        self.timer_var = tk.StringVar(value="25:00")
        ttk.Label(center, textvariable=self.timer_var, style="Timer.TLabel").pack()

        buttons = ttk.Frame(timer)
        buttons.pack(side="right", padx=(8, 0))
        self.start_button = ttk.Button(buttons, text="Start", width=7, style="Primary.TButton", command=self._toggle_running)
        self.start_button.grid(row=0, column=0, padx=(0, 2))
        ttk.Button(buttons, text="Skip", width=7, style="Compact.TButton", command=self._skip).grid(row=0, column=1, padx=1)
        ttk.Button(buttons, text="Reset", width=7, style="Compact.TButton", command=self._reset).grid(row=0, column=2, padx=(2, 0))

        seq_header = ttk.Frame(self)
        seq_header.pack(fill="x", pady=(2, 2))
        ttk.Label(seq_header, text="Sequence", style="Section.TLabel").pack(side="left")
        ttk.Label(seq_header, text="Edit fields directly", style="Muted.TLabel").pack(side="right")
        ...
        controls.pack(side="right", fill="y", padx=(4, 0))
        ttk.Button(controls, text="+", width=2, command=self._add_block).pack(pady=0)
        ttk.Button(controls, text="−", width=2, command=self._delete_selected).pack(pady=0)
        ttk.Separator(controls).pack(fill="x", pady=2)
        ttk.Button(controls, text="↑", width=2, command=lambda: self._move_selected(-1)).pack(pady=0)
        ttk.Button(controls, text="↓", width=2, command=lambda: self._move_selected(1)).pack(pady=0)

        settings = ttk.Frame(self)
        settings.pack(fill="x", pady=(4, 0))

        ttk.Label(settings, text="Cycles", style="Muted.TLabel").pack(side="left")
        self.cycles_spin = tk.Spinbox(settings, from_=0, to=9999, width=4, bd=1, relief="solid", font=("Helvetica", 9))
        self.cycles_spin.pack(side="left", padx=(4, 4))
        ...
        ttk.Label(settings, text="0 = forever", style="Muted.TLabel").pack(side="left")
    ...
    def _refresh_editor(self) -> None:
        ...
        ttk.Label(header, text="Minutes", style="Muted.TLabel").grid(row=0, column=1, sticky="w", padx=(8, 0))
    ...
    def _build_sequence_row(self, index: int, block: Block) -> None:
        ...
        position = ttk.Label(row, text=f"{index + 1}.", width=3, anchor="e", style="Muted.TLabel")
        ...
        name_entry.grid(row=0, column=1, sticky="ew", padx=(4, 6))

        minutes_var = tk.StringVar(value=self._format_minutes(block.minutes))
        minutes_entry = ttk.Entry(row, textvariable=minutes_var, width=7)
